...
export default defineConfig({
  ...
  plugins: [
    ...
    tailwindcss(),
    VitePWA({
      // ⚠️ `autoUpdate` — ההחלטה התהפכה מ-`prompt`, אחרי שמשתמש נתקע.
      //
      // ב-`prompt` גרסה חדשה הותקנה ו*חיכתה* לאישור. בסימולציה מול האתר
      // החי הבאנר לא הופיע בכלל (מרוץ — ראה UpdatePrompt.tsx), והמשתמש
      // נשאר על הגרסה הישנה בכל פתיחה בלי שום סימן.
      //
      // `autoUpdate` מוסיף ל-sw.js את skipWaiting + clientsClaim (נבדק
      // בבנייה): הגרסה החדשה משתלטת מיד, בלי תלות בקוד הדף.
      //
      // ⚠️ מה ש*לא* השתנה: אין טעינה מחדש בכוח באמצע הזנת עסקה. registerSW.js
      // שהתוסף מזריק רק רושם את העובד (נבדק), ו-UpdatePrompt מחליט — טוען
      // מחדש רק בעלייה, ובאמצע שימוש מציע באנר.
